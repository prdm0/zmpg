...
#'
#' The argument `c = NULL` is the default. Thus, the function [acceptance_rejection()] estimates the value of c using the optimization algorithm [lbfgs::lbfgs()]. For more details, see [lbfgs::lbfgs()].
#' If a value of `c` is provided, the function [acceptance_rejection()] will use this value to generate the random observations. An inappropriate choice of c can lead to low efficiency of the acceptance and rejection method.
#'
#' In Unix-based operating systems, the function [acceptance_rejection()] can be executed in parallel. To do this, simply set the argument `parallel = TRUE`.
#' The function [acceptance_rejection()] utilizes the [parallel::mclapply()] function to execute the acceptance and rejection method in parallel.
